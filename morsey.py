...
def main():
    Morse = True                  #Record past letter.
    output = ''                     #The output string.
    morse = ''
    #message = raw_input('Enter message: ').lower() + ' '        #Message to lower case.
    message = sys.argv[0].lower() + ' '
    
    for i in message:                                   #For each character in message:
        if i != '.' and i != '-' and i != ' ':                #If not Morse, run text to Morse.
            Morse = False
    if Morse == True:                          #If Morse,
        ...
        for i in message:               #If it's not Morse, then it must be characters, so for each character:
